...
def parse_page(html, file_name, products_per_page, all_products)
    if all_products >= 25
        (0...products_per_page).each do |product_num|
            parse_product(html, file_nmae, product_num)
        end
    else
...
def get_info(html)
    ...
    return [product_name, product_img, weights, prices]
end
...
def show_data (product, file_name)
    (0...product[2].size).each do |i|
        ...
        current_product = [product_name, product_price, product_image]
        write_to_csv(file_name, current_product)
